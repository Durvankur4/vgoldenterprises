        ...
        <div className="block md:hidden overflow-x-auto pb-4" style={{ scrollBehavior: "smooth" }}>
          <div className="flex gap-3 px-2 w-max" ref={mobileScrollRef}>
            {filteredProducts.map((product) => (
              <Card
                key={product.id}
                className="w-[260px] h-[380px] border rounded-xl shadow-sm flex-shrink-0 relative"
              >
                ...
                <CardContent className="p-4 pb-20 text-left">
                  ...
                  <div className="flex flex-wrap mb-3">
                    {product.specs.map((s, i) => (
                      <span key={i} className="text-sm mr-3 mb-2">{s}</span>
                    ))}
                  </div>
      ...
      {selectedProduct && (
        ...
        >
          ...
          <div
            ...
            className="bg-white rounded-xl shadow-xl max-w-[90vw] md:w-[800px] w-full max-h-[90vh] flex flex-col md:flex-row overflow-hidden relative"
          >
            <button
              onClick={() => setSelectedProduct(null)}
            ...
            <div
              className="w-full md:w-1/2 bg-black flex items-center justify-center cursor-pointer"
              ...
              <div>
                ...
                <div className="flex flex-wrap mb-5">
                  {selectedProduct.specs.map((s, i) => (
                    <span key={i} className="mr-3 mb-2 text-sm">{s}</span>
                  ))}
                </div>
              </div>
              <Button
            ...
            >
              <div className="relative max-w-[90vw] max-h-[90vh] w-full">
                <button
                  ...
                  className="absolute top-4 right-4 z-50 bg-black bg-opacity-60 text-white text-3xl rounded-full px-3 py-1"
                >×</button>
                <img
                  src={selectedProduct.image}
                  alt={selectedProduct.name}
